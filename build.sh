...
function cleanBuildCache() {
    if [[ "$CLEAN_CACHE" == "true" ]]; then
        echo -e "${COLOR_LIGHT_BLUE}Cleaning build cache...${COLOR_RESET}"
...
function buildTarget() {
    ...
    echo -e "${COLOR_LIGHT_MAGENTA}Building for ${rust_target}...${COLOR_RESET}"
    
    # Clean cache if requested
    cleanBuildCache "$rust_target"
    ...
    [[ "$PROFILE" == "release" ]] && build_cmd="$build_cmd --release"
    [[ -n "$FEATURES" ]] && build_cmd="$build_cmd --features $FEATURES"
    [[ "$NO_DEFAULT_FEATURES" == "true" ]] && build_cmd="$build_cmd --no-default-features"
    [[ "$ALL_FEATURES" == "true" ]] && build_cmd="$build_cmd --all-features"
    [[ -n "$PACKAGE" ]] && build_cmd="$build_cmd --package $PACKAGE"
    [[ -n "$BIN_TARGET" ]] && build_cmd="$build_cmd --bin $BIN_TARGET"
    [[ "$WORKSPACE" == "true" ]] && build_cmd="$build_cmd --workspace"
    [[ -n "$MANIFEST_PATH" ]] && build_cmd="$build_cmd --manifest-path $MANIFEST_PATH"
    [[ "$VERBOSE" == "true" ]] && build_cmd="$build_cmd --verbose"
    [[ -n "$ADDITIONAL_ARGS" ]] && build_cmd="$build_cmd $ADDITIONAL_ARGS"
    ...
    echo -e "  ${COLOR_LIGHT_CYAN}${build_cmd}${COLOR_RESET}"
    
    local start_time=$(date +%s)
    
    # Execute build with environment variables
    if [[ ${#build_env[@]} -gt 0 ]]; then
        env "${build_env[@]}" $build_cmd || return $?
    else
        $build_cmd || return $?
    fi
...
echo -e "${COLOR_LIGHT_BLUE}Build configuration:${COLOR_RESET}"
...
for target in "${TARGET_ARRAY[@]}"; do
    ...
    buildTarget "$target" || {
        echo -e "${COLOR_LIGHT_RED}Build failed for target: ${target}${COLOR_RESET}"
        exit 1
    }
done
...
echo -e "${COLOR_LIGHT_GRAY}$(printSeparator)${COLOR_RESET}"
echo -e "${COLOR_LIGHT_GREEN}All builds completed successfully!${COLOR_RESET}"
